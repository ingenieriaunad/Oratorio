(async()=>{
    ...
    const getCountReading=(count)=>{
        ...
        if(count>=62&&count<=71){
            return count-62;
        }
    }
    const getMisteryNumber=(count)=>{
        if(count>=0&&count<=12){
            return 0;
        }
        ...
        if(count>=31&&count<=43){
            return 2;
        }
        if(count>=46&&count<=58){
            return 3;
        }
        if(count>=61&&count<=73){
            return 4;
        }
    }
    ...
    const setRosaryContent=(count)=>{
        ...
        if(isInRange(count, rangeHailMary)){
            if(count<76){
                pray.innerHTML=`${prays.thoughtfulReadings[dayName][getMisteryNumber(count)][getCountReading(count)]}<br><span class="text-primary">${prays.hailMary}</span>`;
            }
            else{
                pray.innerHTML=`<span class="text-primary">${prays.hailMary}</span>`;
            }
        }
